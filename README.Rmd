...
evalHeadlineWordsTrain <- headlineWordsTrain[spl==T,]
evalHeadlineWordsTest <- headlineWordsTrain[spl==F,]
```
...
newsTrainMeta <- data.frame(
    ...
    NewsDesk = factor(newsTrain$NewsDesk, levels=levels(as.factor(newsTrain$NewsDesk))),
    SectionName = factor(newsTrain$SectionName, levels=levels(as.factor(newsTrain$SectionName))),
    SubsectionName = factor(newsTrain$SubsectionName, levels=levels(as.factor(newsTrain$Subsection)))
    )
newsTestMeta <- data.frame(
    NewsDesk = factor(newsTest$NewsDesk, levels=levels(as.factor(newsTrain$NewsDesk))),
    SectionName = factor(newsTest$SectionName, levels=levels(as.factor(newsTrain$SectionName))),
    SubsectionName = factor(newsTest$SubsectionName, levels=levels(as.factor(newsTrain$Subsection)))
    )
```
...
# Subset seems to remove factor levels so we subset dataframes by index
evalNewsTrain <- newsTrainMeta[spl==T,]
evalNewsTest <- newsTrainMeta[spl==F,]
```
...
## Trees and Random Forests on metadata  
...
The three is clearly too coarse. It classifies popular articles as being just
...
```{r}
metaCartCvModel <- rpart(Popular ~ ., data = evalNewsTrain, cp = 0.01)
...
```{r}
evalNewsMetaTextTrain <- newsTrainMetaText[spl==T,]
evalNewsMetaTextTest <- newsTrainMetaText[spl==F,]
```
...
This time we get `cp=0.02`. Let's build the model.  

```{r}
metaTextCartCvModel <- rpart(Popular~., data=evalNewsMetaTextTrain, cp=0.02)
...
So let's add both things, day of the week and month to our dataset.  

```{r}
DayOfTheWeek <- weekdays(as.Date(newsTrain$PubDate))
MonthOfTheYear <- months(as.Date(newsTrain$PubDate))
newsTrainMetaTextDate <- cbind(newsTrainMetaText, DayOfTheWeek, MonthOfTheYear)
evalMetaTextDateTrain <- newsTrainMetaTextDate[spl==T,]
evalMetaTextDateTest <- newsTrainMetaTextDate[spl==F,]
```

And let's compare the results with other linear models we trained before.  

```{r}
metaTextDateLogModel <- glm(Popular~., data=evalMetaTextDateTrain, family=binomial)
summary(metaTextDateLogModel)
```

It seems that the day of the week pays an important role in popularity, but not
the month of the year. At least for logistic regression.  

But let's calculate the `ac` value.  

```{r}
metaTextDateLogPred <- predict(metaTextDateLogModel, newdata=evalMetaTextDateTest, type="response")
metaTextDateLogRocr <- prediction(metaTextDateLogPred, evalMetaTextDateTest$Popular)
metaTextDateLogAuc <- as.numeric(performance(metaTextDateLogRocr, "auc")@y.values)
metaTextDateLogAuc
```

And we see a slight improvement! Let's build our finest model so far, the random
forest.  

```{r}
metaTextDateRfModel <- randomForest(Popular~., data=evalMetaTextDateTrain)
metaTextDateRfPred <- predict(metaTextDateRfModel, newdata=evalMetaTextDateTest)
metaTextDateRfRocr <- prediction(metaTextDateRfPred, evalMetaTextDateTest$Popular)
metaTextDateRfAuc <- as.numeric(performance(metaTextDateRfRocr, "auc")@y.values)
metaTextDateRfAuc
```

That also improves the model without dates. Let's use this one to save our 
results file to Kaggle.  


```{r}
metaTextDateRfModel <- randomForest(Popular~., data=newsTrainMetaTextDate)
DayOfTheWeek <- factor(weekdays(as.Date(newsTest$PubDate)), 
                         levels=levels(newsTrainMetaTextDate$DayOfTheWeek))
MonthOfTheYear <- factor(months(as.Date(newsTest$PubDate)), 
                         levels=levels(newsTrainMetaTextDate$MonthOfTheYear))
newsTestMetaTextDate <- cbind(newsTestMeta, headlineWordsTest, DayOfTheWeek, MonthOfTheYear)
metaTextDateRfPred <- predict(metaTextDateRfModel, newdata=newsTestMetaTextDate)
mySubmission <- data.frame(
    UniqueID = newsTest$UniqueID, 
    Probability1 = abs(metaTextRfPred)
    )
write.csv(mySubmission, "SubmissionMetaTextDateRF.csv", row.names=FALSE)
```

# About times of the day?  

see https://courses.edx.org/c4x/MITx/15.071x_2/asset/Unit7_Crime.R
...
In order to use TF-IDF with the `tm` library command `DocumentTermFrequency` we
just need to include a control term.  

```{r}
control = list(weighting = weightTfIdf)
```

# Add additional corpora  
